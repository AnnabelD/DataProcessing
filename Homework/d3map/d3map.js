...
window.onload = function(){
	var data_format = {}
	var data = d3.json("world.json", function(error,data){
		for (var i = 0; i < data.length; i++){
			data[i][" population"] = parseInt(data[i][" population"]);
			for (var f = 0, len = country_codes.length; f < len - 1; f++){
				var index = country_codes[f].indexOf(data[i]["country"]);
				if (index != -1){
					data[i]["name"] = data[i]["country"];
					data[i]["country"] = country_codes[f][1];
					data[i]["fillKey"] = key(data[i][" population"]);
					data_format[data[i]["country"]] = {fillKey: data[i]["fillKey"],
					"name": data[i]["name"],
		...
		var map = new Datamap({element: document.getElementById('map'),
	        ...
	        geographyConfig: {
                ...
                if (!data) { return[ '<div class="hoverinfo">',
                    ...
                    '<br>Population size: <strong>', "No data", '</strong>',
                    '</div>'].join('');; }
                // tooltip content
